...
GLASS_STYLES = {"liquid": "Liquid Glass — tím / ngọc lam", "crystal": "Crystal Glass — cam / xanh bạc",
                "basic": "Basic — nút phẳng xanh dương"}
BORDER_STYLES = {"glass": "Viền kính", "solid": "Viền đơn sắc", "none": "Không viền"}
DEFAULT_APPEARANCE = {"glass_style": "liquid", "glass_tint": "smoke", "glass_transparency": 100, "glass_outer_transparency": 100, "glass_blur": False,
                      "border_style": "glass", "border_width": 1.5, "border_color": "#a788f3", "border_opacity": 100}
...
def appearance_options(settings=None):
    ...
    border_style = settings.get("border_style", "glass")
    if not isinstance(border_style, str) or border_style not in BORDER_STYLES:
        border_style = "glass"
    try:
        border_width = float(settings.get("border_width", 1.5))
        if not math.isfinite(border_width):
            border_width = 1.5
    except (TypeError, ValueError, OverflowError):
        border_width = 1.5
    border_color = settings.get("border_color", "#a788f3")
    color = QColor(border_color) if isinstance(border_color, str) else QColor()
    border_color = color.name() if color.isValid() else "#a788f3"
    try:
        border_opacity = int(settings.get("border_opacity", 100))
    except (TypeError, ValueError, OverflowError):
        border_opacity = 100
    return {"glass_style": style, "glass_tint": tint, "glass_transparency": max(0, min(100, transparency)),
            ...
            "border_style": border_style, "border_width": max(0.5, min(4.0, border_width)),
            "border_color": border_color, "border_opacity": max(0, min(100, border_opacity))}


def is_light_appearance(settings=None):
    options = appearance_options(settings)
    return options["glass_tint"] == "light" or (options["glass_style"] in {"crystal", "basic"} and options["glass_tint"] != "black")
...
def native_glass_tint(settings=None):
    ...
    if options["glass_style"] == "basic" and options["glass_tint"] not in {"black", "light"}:
        color = QColor("#eaf1ff")
...
def apply_theme(app: QApplication, settings=None):
    ...
    basic = options["glass_style"] == "basic"
    if basic:
        if crystal:
            base, surface = ("#ffffff" if light else "#eaf1ff"), "#ffffff"
        accent = "#007bff"
    foreground, muted = ("#283744", "#526773") if crystal else ("#f4f4f0", "#c9c8c3")
    app.setProperty("glassStyle", "crystal" if crystal else "liquid")
    app.setProperty("buttonStyle", options["glass_style"])
    for key in ("border_style", "border_width", "border_color", "border_opacity"):
        app.setProperty(key, options[key])
    ...
    preferred = ("Arial", "Liberation Sans", "Noto Sans", "DejaVu Sans", "Segoe UI")
...
QWidget#glassTitleBar {{ background: transparent; border: 0; }}
...
"""
    if basic:
        basic_style = (Path(__file__).resolve().parents[1] / "resources" / "basic.qss").read_text(encoding="utf-8")
        colors = {"TEXT": foreground, "SURFACE": "#ffffff" if crystal else "#202a38",
                  "BORDER": "#cbd7eb" if crystal else "#45566d",
                  "HOVER": "#e5efff" if crystal else "#293b54",
                  "SELECTED": "#d4e5ff" if crystal else "#183d69",
                  "MUTED": "#8693a8" if crystal else "#8090a5"}
        for key, value in colors.items():
            basic_style = basic_style.replace(f"@{key}@", value)
        stylesheet += basic_style
    stylesheet += (Path(__file__).resolve().parents[1] / "resources" / "typography.qss").read_text(encoding="utf-8")
    stylesheet += f'\n* {{ font-family: "{family}"; }}\nQPlainTextEdit#logConsole {{ font-family: "{family}"; }}\n'
    if options["border_style"] != "glass" or options["border_width"] != 1.5 or options["border_opacity"] != 100:
        width = 0 if options["border_style"] == "none" else options["border_width"]
        color = QColor(options["border_color"] if options["border_style"] == "solid" else ("#526773" if crystal else "#ffffff"))
        alpha = round(255 * options["border_opacity"] / 100 * (0.35 if options["border_style"] == "glass" else 1))
        border = f"{width}px solid rgba({color.red()},{color.green()},{color.blue()},{alpha})"
        stylesheet += f"""
QWidget#sidebar, QWidget#contentShell, QWidget#card, QWidget#metricCard, QWidget#quickPanel,
QWidget#metricCard[metric="contents"], QWidget#metricCard[metric="images"], QWidget#metricCard[metric="videos"], QWidget#metricCard[metric="posts"],
QWidget#timelineStep, QGroupBox, QTabWidget::pane, QLabel#mediaPreview,
QPushButton, QPushButton#primaryButton, QPushButton#linkButton, QPushButton:hover, QPushButton:pressed,
QLineEdit, QTextEdit, QPlainTextEdit, QComboBox, QDateTimeEdit, QSpinBox, QDoubleSpinBox, QDateEdit, QTimeEdit {{ border: {border}; }}
QWidget#glassTitleBar QPushButton {{ border: 0; }}
"""
